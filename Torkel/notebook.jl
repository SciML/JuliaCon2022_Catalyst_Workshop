...
# ╔═╡ cf9a02ba-feaf-11ec-085e-792c751d3f75
using Plots, BifurcationKit,  Setfield, Latexify

# ╔═╡ f7152e4a-fb0b-443d-afcb-1ac7a390f617
using Catalyst

# ╔═╡ 57533bbf-6a1b-4e29-a76a-211a3d5477f3
using DifferentialEquations

# ╔═╡ 9a8918ff-4829-425e-b6f1-10eef1503030
### Section 1: Simple Birth/Death Example ###

# ╔═╡ c83c76fe-d0c7-45d3-9a06-ffdd7c6ca663
bd_model = @reaction_network begin
	p, 0 --> X
	d, X --> 0
end p d;

# ╔═╡ 42cdf525-52c0-44a4-a76c-bf681beeaff3
u0 = [1.0];

# ╔═╡ a51c5644-0fbd-4843-a0a0-4082a6d8f7af
tspan = (0.0,100.0);

# ╔═╡ ada23778-f028-443c-9e07-7663765b232d
p_vals = [1.0,0.1];

# ╔═╡ c9e0085c-d59c-4862-a9ae-177d24c50fa0
oprob = ODEProblem(bd_model,u0,tspan,p_vals);

# ╔═╡ 77fa042f-1a1e-4d6f-b4a9-58efc10999c3
osol = solve(oprob);

# ╔═╡ 40bcd8b1-7f50-40a2-8958-970bfccb4318
plot(osol)

# ╔═╡ b6b3f86a-776f-4617-9c84-e9f4b81de95e
sprob = SDEProblem(bd_model,u0,tspan,p_vals);

# ╔═╡ 76315917-6f73-4558-a2f2-264d50e8aedd
ssol = solve(sprob);

# ╔═╡ 897678b5-8551-40c3-b7cf-c23653330e46
plot(ssol)

# ╔═╡ a04eac61-999d-414c-8709-765cbd6644f8
dprob = DiscreteProblem(bd_model,u0,tspan,p_vals);

# ╔═╡ f2a21adc-980e-446a-a945-3a40e3a67926
jprob = JumpProblem(bd_model,dprob,Direct());

# ╔═╡ 812b6217-1574-4cb0-9411-3d71f1e3ca7f
jsol = solve(jprob,SSAStepper());

# ╔═╡ d0e73deb-e95e-4c9c-ae32-ace9bb9447de
plot(jsol)

# ╔═╡ 0ab417c3-eb49-4147-9763-b1281dfb5ed6
### Section 2: DSL Details ###

# ╔═╡ aba42490-7f4c-4ae6-ae67-c94096b8bb09
# Bundling of reactions.

# ╔═╡ 19afa7cc-09ad-4b6a-9773-7866e832b9c4
example_network_1 = @reaction_network begin
	p, 0 --> X
	d, X --> 0
	k1, X + Y --> XY
	k2, 2X --> X2
...
# ╔═╡ 546d4e68-6614-42f5-b8db-8707576e5bd8


# ╔═╡ 55f3960e-9788-404d-b6a3-a96e78de2949
bd_model_alt = @reaction_network begin
	(p,d), 0 <--> X
end p d;

# ╔═╡ e84b5656-e408-4eaf-a159-58f2d948f8e9
example_network_2 = @reaction_network begin
	p, 0 --> X
	p, 0 --> Y
	kB, X + Y --> XY
	d, XY --> 0 
...
# ╔═╡ b1945e9d-c62e-4857-863e-0fc1bbc947ec
example_network_2_alt = @reaction_network begin
	p, 0 --> (X,Y)
	kB, X + Y --> XY
	d, XY --> 0 
end p kB d;

# ╔═╡ 581f6c4e-5f93-4de1-9bb0-d9cf7b1c2de6
example_network_3 = @reaction_network begin
	p1, 0 --> X
	p2, 0 --> Y
	kB, X + Y --> XY
	d, XY --> 0 
...
# ╔═╡ a630619e-251e-463f-a258-68ab2509fefe
example_network_3_alt = @reaction_network begin
	(p1,p2), 0 --> (X,Y)
	kB, X + Y --> XY
	d, XY --> 0 
end p1 p2 kB d;

# ╔═╡ 51d9c434-5800-46ce-a510-418ad419bfb1
example_network_4 = @reaction_network begin
	kX, X1 --> X2
	kY, Y1 --> Y2
end kX kY;

# ╔═╡ 769402b3-50bf-4bfb-9b95-cf43f7e5e86a
example_network_4_alt = @reaction_network begin
	(kX,kY), (X1,Y1) --> (X2,Y2)
end kX kY;

# ╔═╡ 045f9e18-6e24-4187-8f6c-7b92380c0b76
example_network_5 = @reaction_network begin
	p, 0 --> (X,Y)
	kB, X + Y --> XY
	d, (X,Y,XY) --> 0
end p kB d;

# ╔═╡ 862d27ef-ee9b-40e2-aa18-0c99bff3fe41
example_network_6 = @reaction_network begin
	((p1,p2),d), 0 <--> (X,Y)
end p1 p2 d;

# ╔═╡ 6914f44b-7a1d-4484-9eb7-6eaca9556257
# Non-cosntant Reaction Rates

# ╔═╡ 2fd152a1-2494-48b0-bd7c-d7a7ed955595
example_network_7 = @reaction_network begin
	(p/q,d), 0 <--> X
end p q d;

# ╔═╡ c697211f-4893-47ce-b131-4896d87d788d
f(p) = 3*p^2;

# ╔═╡ 5c8a9fdc-ee3a-4347-a472-f93253cc5c3c
example_network_8 = @reaction_network begin
	(f(p),d), 0 <--> X
end d;

# ╔═╡ f6b30389-774e-445a-b384-9f015b9eef07
example_network_9 = @reaction_network begin
	(X,d), 0 <--> X
end d;

# ╔═╡ a0da5caa-2752-431a-9aee-b1016316b4a2
example_network_10 = @reaction_network begin
	(f(X),d), 0 <--> X
end d;

# ╔═╡ 95a25ba7-edeb-48a6-b56a-dfd665a3d32e
example_network_11 = @reaction_network begin
	hill(X,v,K,n), 0 --> X
	d, X --> 0
end v K n d;

# ╔═╡ a6fa263e-807f-40e6-959e-e4f15650f820
example_network_12 = @reaction_network begin
	mm(X,v,K), 0 --> X
	d, X --> 0
end v K n d;

# ╔═╡ 471331d8-18bd-47a6-b907-bab23e1bd1e7
example_network_13 = @reaction_network begin
	p/t, 0 --> X
	d, X --> 0
end p d;

# ╔═╡ b9f6efa8-c7b3-4a0c-8c66-2e8ac50fb9cc
# Forgot to list a parameter.

# ╔═╡ 854a2246-65fa-4058-b599-f7d62bd4095b
example_network_14 = @reaction_network begin
	(p,d), 0 <--> X
end d;

# ╔═╡ c9869ba6-78f2-44aa-97e3-004606e52c4f
# Alternative Symbols

# ╔═╡ d390eb02-023d-4501-ae63-3eef5538394b
example_network_15 = @reaction_network begin
	p, ∅ --> X
	p, ∅ → X
	d, ∅ ← X
	(p,d), ∅ ↔ X
...
# ╔═╡ a786c5c1-b05d-4b61-81db-4d22a7d30d69
### Section 4: Advanced Simulation Options ###

# ╔═╡ 07e288c1-a10e-4e8e-b01e-5eafcb6583b1
# Reminder, simple ODESimulation:

# ╔═╡ c33efd10-7861-4232-87d3-68ae81bcf2d5
pos_feedback = @reaction_network begin
	v0 + hill(i*X,v,K,n), 0 --> X
	1, X --> 0
end i v0 v K n;

# ╔═╡ 6f9edb5c-f042-4aa4-8b6c-7a482f84a6bb
u0_pf = [2.0]; tspan_pf = (0.0,5.0); p_vals_pf = [2.0,0.1,10.0,0.5,2];

# ╔═╡ 348cf8e5-8b68-4acf-81e2-6c144a6100dd
oprb_pf = ODEProblem(pos_feedback,u0_pf,tspan_pf,p_vals_pf);

# ╔═╡ 43b3a9e6-a0d9-4441-83ac-aa9bc0734b72
osol_pf = solve(oprb_pf,Rosenbrock23());

# ╔═╡ e4c11e82-8fd3-4ddc-a176-c113e6039c2e
plot(osol_pf)

# ╔═╡ 7b2cae59-6e1e-426d-9ede-acf501cebcdf
plot(osol_pf,xguide="Time",yguide="Concentration",framestyle=:box,label="",lw=4)

# ╔═╡ 629351c1-efc5-40ee-b231-7243d525585f
sprb_pf = SDEProblem(pos_feedback,u0_pf,tspan_pf,p_vals_pf);

# ╔═╡ 51118e41-f387-4079-8778-7b4f0290f37b
ssol_pf = solve(sprb_pf,ImplicitEM());

# ╔═╡ 9a1530bc-f28c-4fc2-bb13-8efebb9ebad3
plot(ssol_pf)

# ╔═╡ e46a113a-9def-4968-a5ac-5b60af0e6c79
pos_feedback_noise_scaling = @reaction_network begin
	v0 + hill(i*X,v,K,n), 0 --> X
	1, X --> 0
end i v0 v K n η;

# ╔═╡ 5d962bd9-a22b-4db4-bcef-457942c3a9bb
p_vals_pf_ns = [2.0,0.1,10.0,0.5,2,0.2];

# ╔═╡ 218192ce-a592-40ed-97e4-37d893f30f99
sprb_pf_ns = SDEProblem(pos_feedback_noise_scaling,u0_pf,tspan_pf,p_vals_pf_ns;noise_scaling=(@parameters η)[1]);

# ╔═╡ feba7747-b34c-4a60-be19-ef14fff9345e
ssol_pf_ns = solve(sprb_pf_ns,ImplicitEM());

# ╔═╡ 8d784887-06a1-4ba9-a5f3-5348a51f1819
plot(ssol_pf_ns)

# ╔═╡ 0d1a6a35-1fea-4e51-b2f4-ccd97421aba8
dprb_pf = DiscreteProblem(pos_feedback,[1],tspan_pf,p_vals_pf);

# ╔═╡ ea5a1639-0683-46e1-bf1b-69f57987abac
jprb_pf = JumpProblem(pos_feedback,dprb_pf,Direct());

# ╔═╡ 5eccdbb7-7210-4ce8-9f3f-2afd2bfe88de
jsol_pf = solve(jprb_pf,SSAStepper());

# ╔═╡ eadcafa5-9e21-4ee7-9212-ca5afc1dcfcc
plot(jsol_pf)

# ╔═╡ da437ad5-9ea3-4eec-b986-4444880ae639
affect!(integrator) = integrator.p[1] = 1;

# ╔═╡ de8771ec-34d6-4794-a9e1-cd18762faa24
cb = PresetTimeCallback([5.0],affect!);

# ╔═╡ 44209a29-07c7-4266-bd70-e9c1987c2ac0
p_vals_pf_cb = [0.0,0.1,10.0,0.5,2];

# ╔═╡ 6b040c6a-7f41-4ee0-a260-508ef13d5ff0
dprb_pf_cb = DiscreteProblem(pos_feedback,[0],(0.0,25.0),p_vals_pf_cb);

# ╔═╡ ccba48a5-c793-42c1-9b77-0466124ec10d
jprb_pf_cb = JumpProblem(pos_feedback,dprb_pf_cb,Direct());

# ╔═╡ 85bd56b1-9504-48a3-97c6-c6f2172ab364
jsol_pf_cb = solve(jprb_pf_cb,SSAStepper();callback=cb);

# ╔═╡ 664ba61e-adbd-4d2e-8ed1-fac74ba3c347
plot(jsol_pf_cb)

# ╔═╡ c027d094-133a-457f-925f-0dd774e5f423
prob_func(prob,i,repeat) = remake(prob,p=deepcopy(prob.prob.p));

# ╔═╡ 8e0012b0-65b5-4b36-bded-8fe5ae80fde5
eprob = EnsembleProblem(jprb_pf_cb,prob_func=prob_func,safetycopy=false);

# ╔═╡ 278f5754-f3d4-4ba5-b9ae-5512e1aa9a2d
esol_pf_cb = solve(eprob,SSAStepper();trajectories=25,callback=cb)

# ╔═╡ 9089f21b-1ce1-41c8-b2b6-776aea479d52
plot(esol_pf_cb)

# ╔═╡ 4f20cbe2-ede5-418b-8004-26cf6237b569
plot(esol_pf_cb,linealpha=0.7)

# ╔═╡ 574b12b5-54ae-4d8c-adb8-83b3e1c89485
birth_death_model = @reaction_network begin
	(p,d), 0 <--> X
end p d;

# ╔═╡ 4496e1e7-0a5e-4004-be51-87d29b03fb61
u0_birth_death = [1.0]; tspan_birth_death = (0.0,10.0); p_vals_birth_death = [1.0,1.0];

# ╔═╡ cceb32d6-d89f-4c63-a282-9f2ae66146cb
sprob_birth_death = SDEProblem(birth_death_model,u0_birth_death,tspan_birth_death,p_vals_birth_death);

# ╔═╡ 0072f9a0-3e4d-4575-8b85-3929e8a72795
ssol_birth_death_1 = solve(sprob_birth_death,ImplicitEM());

# ╔═╡ 950c8cee-413e-40a5-a7c7-31598cae1ebd
plot(ssol_birth_death_1)

# ╔═╡ 02df46d5-3879-447c-962a-421e9141299d
ssol_birth_death_2 = solve(sprob_birth_death,ImplicitEM();callback=PositiveDomain());

# ╔═╡ 30ec3025-bea7-45fa-ad8c-d3fc95d46661
plot(ssol_birth_death_2)

# ╔═╡ 7ae0b681-8f59-4e2d-a0c4-4e4b3059eb3e
### Section 7: Steady State and Bifurcation Analysis ###

# ╔═╡ 999e27af-6af0-413c-8864-6997c17e439f
...
git-tree-sha1 = "afe86a756a89b378c5cf59f672338d80ea0b7ffe"
